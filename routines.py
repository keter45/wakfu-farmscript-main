...
from utils import get_closest_point, toss_coin, move_and_click, find_icon_and_click, get_action_icon_by_resource
...
def advanced_fisherman_actions():
    accuracy = 0.88
    fish_locations = (
        list(auto.locateAllOnScreen(const.FISHERMAN_RES_PATH + "fish1.png", confidence=accuracy)) +
        list(auto.locateAllOnScreen(const.FISHERMAN_RES_PATH + "fish2.png", confidence=accuracy)) +
        list(auto.locateAllOnScreen(const.FISHERMAN_RES_PATH + "fish3.png", confidence=accuracy)) +
        list(auto.locateAllOnScreen(const.FISHERMAN_RES_PATH + "fish4.png", confidence=accuracy))
    )
...
def advanced_farming_actions():
    ...
    try:
        seeds_locations = list(auto.locateAllOnScreen(
    ...
    try:
        resource_locations = list(auto.locateAllOnScreen(
...
def advanced_lumberjack_actions():
    ...
    mature_locations = list(auto.locateAllOnScreen(
        const.LUMBERJACK_RES_PATH + _get_img_name(resource, "mature"),
        confidence=0.88,
    ))

    little_locations = list(auto.locateAllOnScreen(
